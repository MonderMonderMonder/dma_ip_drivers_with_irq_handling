...
int main(int argc, char **argv) {
    ...
    if (argc <= 2) {
        printf("Usage: ./mmr <number of counter mm register reads> <result csv file name>");
        exit(1);
    }
    ...
    int n = atoi(argv[1]);
	int counter_reads[n];
    for (int i = 0; i < n; i++) counter_reads[i] = 0;

    for (int i = 0; i < n; i++) counter_reads[i] = *(reg_girq_counter); // TODO justify implementation choice with disassembler
        
    struct stat st = {0};
    if (stat("./results", &st) && mkdir("./results", 0777)) exit(1);
    char file_path [256];
    snprintf(file_path, sizeof(file_path), "./results/%s.csv", argv[2]);
    FILE *file = fopen(file_path, "w");
    if (file == NULL) {
        printf("Error opening file.\n");
        exit(1);
    }
    for (int i = 0; i < n; i++) fprintf(file, "%u\n", counter_reads[i]);
    fclose(file);
